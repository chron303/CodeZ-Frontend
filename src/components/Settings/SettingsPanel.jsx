...
function ChangePasswordSection() {
  ...
  function reset() { setCurrentPw(''); setNewPw(''); setConfirmPw(''); setError(''); setSuccess(false); }
  ...
  if (!isEmailUser) {
    ...
    const name = provider === 'google.com' ? 'Google' : provider === 'phone' ? 'Phone' : 'your login provider';
    return (
  ...
  return (
    <div className="game-card p-3 space-y-2.5">
      ...
      <PasswordInput value={currentPw} onChange={e => setCurrentPw(e.target.value)}
        placeholder="Current password" disabled={loading} />
      <PasswordInput value={newPw} onChange={e => setNewPw(e.target.value)}
        placeholder="New password (min 8 chars)" disabled={loading} />
      ...
      <div className="flex gap-2 pt-1">
        <button onClick={handleSubmit} disabled={loading}
          ...
          {loading
            ? <><Loader className="w-3.5 h-3.5 animate-spin" /> Updating…</>

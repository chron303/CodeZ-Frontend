...
import { ChevronLeft, ChevronRight, CheckCircle, TrendingUp, Loader, Zap } from 'lucide-react';
...
function ReportCard({ report, index, activeIndex, total, onClick }) {
  ...
  return (
    <div onClick={onClick} className="absolute cursor-pointer select-none"
      style={{ width:240, left:'50%', marginLeft:-120,
        transform:`translateX(${translateX}px) scale(${scale})`,
        opacity, zIndex, transition:'all 0.45s cubic-bezier(0.34,1.56,0.64,1)' }}>
      <div className={`rounded-2xl border-2 overflow-hidden ${g.border}`}
        style={{ background:'linear-gradient(145deg,#1a1830,#0f0e17)',
          boxShadow: isActive ? `0 20px 60px ${g.glow}35, 0 0 0 1px ${g.glow}20` : '0 8px 24px rgba(0,0,0,0.4)' }}>
        <div className="p-4 text-center" style={{ background:`linear-gradient(135deg,${g.glow}18,transparent)` }}>
          <div className={`inline-flex items-center justify-center w-14 h-14 rounded-xl border-2 text-3xl font-black mb-2 ${g.bg} ${g.border} ${g.text}`}
...
function ReportDetail({ report, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl border bg-game-card"
        style={{ borderColor:g.glow+'40', boxShadow:'0 0 60px '+g.glow+'25',
        ...
        <div className="p-5 text-center border-b border-game-border"
          ...
          <div className={`inline-flex items-center justify-center w-16 h-16 rounded-2xl border-2 text-4xl font-black mb-2 ${g.bg} ${g.border} ${g.text}`}
            style={{ boxShadow:`0 0 25px ${g.glow}45` }}>{report.grade}</div>
          <p className={`text-lg font-bold ${g.text}`}>{report.verdict}</p>
          <p className="text-slate-500 text-xs mt-1">{report.level} · {report.passedCount}/{report.problemCount} solved · Score: {report.overallScore}/100</p>
        </div>
        <div className="p-5 space-y-4">
          {report.summary && <p className="text-sm text-slate-400 leading-relaxed">{report.summary}</p>}
          ...
          <div className="grid grid-cols-2 gap-3">
            {report.strengths?.length > 0 && (
              <div>
                ...
                {report.strengths.map(function(s,i) { return <p key={i} className="text-xs text-slate-500 mb-1 leading-relaxed">{s}</p>; })}
              </div>
            )}
            {report.improvements?.length > 0 && (
              <div>
                <p className="text-xs font-semibold text-yellow-400 mb-2 flex items-center gap-1"><TrendingUp className="w-3 h-3"/> Improve</p>
                {report.improvements.map(function(s,i) { return <p key={i} className="text-xs text-slate-500 mb-1 leading-relaxed">{s}</p>; })}
              </div>
            )}
          </div>
                  ...
                  return (
                    <div key={i} className="p-3 rounded-xl bg-game-surface border border-game-border space-y-1.5">
                      ...
                      <div className="h-1.5 bg-game-card rounded-full overflow-hidden ml-6">
                        <div className="h-full rounded-full" style={{ width:p.score+'%', background:p.score>=80?'#22c55e':p.score>=50?'#eab308':'#ef4444' }}/>
                      </div>
...
export default function MockReportCards() {
  ...
  useEffect(function() {
    if (!user) return;
    // authFetch attaches the token — backend verifies uid from token, not query param
    authFetch('/mock/reports/' + user.uid)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {reports.length > 1 && <span className="text-xs text-slate-600">← → to browse</span>}
      </div>

      {/* Card carousel */}
      <div className="relative overflow-hidden" style={{ height:360, perspective:1000 }}>
        <div className="relative w-full h-full" style={{ transformStyle:'preserve-3d' }}>
          {reports.map(function(report, i) {
            return (
              <ReportCard key={report.id||i} report={report} index={i}
                activeIndex={activeIndex} total={reports.length}
                onClick={function() {
        ...
        {activeIndex > 0 && (
          <button onClick={function() { setActiveIndex(function(i) { return i-1; }); }}
            className="absolute left-2 top-1/2 -translate-y-1/2 z-50 w-8 h-8 rounded-full
              bg-game-card border border-game-border flex items-center justify-center
              text-slate-400 hover:text-slate-200 hover:border-slate-600 transition-colors">
        ...
        {activeIndex < reports.length - 1 && (
          <button onClick={function() { setActiveIndex(function(i) { return i+1; }); }}
            className="absolute right-2 top-1/2 -translate-y-1/2 z-50 w-8 h-8 rounded-full
              bg-game-card border border-game-border flex items-center justify-center
              text-slate-400 hover:text-slate-200 hover:border-slate-600 transition-colors">
      ...
      {reports.length > 1 && (
        <div className="flex justify-center gap-1.5">
          {reports.map(function(_, i) {
            ...
            return (
              <button key={i} onClick={function() { setActiveIndex(i); }}
                className="w-2 h-2 rounded-full transition-all"
                style={{ background:i===activeIndex?g.glow:'#2d2a45', transform:i===activeIndex?'scale(1.4)':'scale(1)' }}/>
            );
          })}
        </div>
      )}

      <p className="text-center text-xs text-slate-700">Tap the front card to view full report</p>

      {detail && <ReportDetail report={detail} onClose={function() { setDetail(null); }}/>}
    </div>
  );
}
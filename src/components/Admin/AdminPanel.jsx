...
// Full admin panel — theme-aware, with tabs for:
//   Problem details, Test cases, Solution, Comments, Hints
...
import { Plus, Pencil, Trash2, Save, X, ChevronDown, ChevronRight,
  ...
  Code2, MessageSquare, Lightbulb, BookOpen, RefreshCw } from 'lucide-react';
import {
...
function ProblemForm({ initial, onSave, onCancel, saving }) {
  const [form,    setForm]    = useState(() => {
    ...
    return {
      ...base,
      ...initial,
      solution: initial.solution || base.solution,
      comment:  initial.comment  || base.comment,
    };
  });
  ...
  function onTestChange(i, field, val) {
    const tcs = form.testCases.map((tc,idx) => idx===i ? {...tc,[field]:val} : tc);
    set('testCases', tcs);
  }
  function addTest() {
  ...
  function removeTest(i) {
    ...
    set('testCases', form.testCases.filter((_,idx)=>idx!==i));
  }

  // Solution code
  function setSolutionLang(lang, code) {
    set('solution', {...(form.solution||{}), [lang]: code });
  }

  // Comment fields
  function setComment(field, val) {
    set('comment', {...(form.comment||{}), [field]: val });
  }
  function setComplexity(field, val) {
    set('comment', {...(form.comment||{}), complexity:{...(form.comment?.complexity||{}), [field]:val}});
  }
  // Hints
  function setHint(i, val) {
    const hints = [...(form.comment?.hints||[])];
    hints[i] = val;
    setComment('hints', hints);
  }
  function addHint() { setComment('hints', [...(form.comment?.hints||[]), '']); }
  function removeHint(i) {
    setComment('hints', (form.comment?.hints||[]).filter((_,idx)=>idx!==i));
  }
  // Walkthrough lines
  function setWalkLine(i, val) {
    const lines = [...(form.comment?.walkthrough||[])];
    lines[i] = val;
    setComment('walkthrough', lines);
  }
  function addWalkLine() { setComment('walkthrough', [...(form.comment?.walkthrough||[]), '']); }
  function removeWalkLine(i) {
    setComment('walkthrough', (form.comment?.walkthrough||[]).filter((_,idx)=>idx!==i));
  }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <Field label="Description">
            <textarea value={form.description} onChange={e=>set('description',e.target.value)}
              rows={5} placeholder="Problem statement…" className={textareaCls}/>
          </Field>
          <Field label="LeetCode URL (optional)">
            <input value={form.url||''} onChange={e=>set('url',e.target.value)}
              placeholder="https://leetcode.com/problems/two-sum/" className={inputCls}/>
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Tags (comma separated)">
              <input value={(form.tags||[]).join(',')} onChange={e=>set('tags',e.target.value.split(',').map(t=>t.trim()).filter(Boolean))}
                placeholder="array, hash-map" className={inputCls}/>
            </Field>
            <Field label="Order">
        ...
        <div>
          ...
          <div className="space-y-2">
            {form.testCases.map((tc,i)=>(
              <TestCaseRow key={tc.id} tc={tc} index={i}
                onChange={onTestChange} onRemove={removeTest}
      ...
      {activeTab==='solution' && (
        <div className="space-y-3">
          <p className="text-xs text-slate-600">
            Paste the solution code here. This will be shown to users in the Solution tab after they attempt the problem.
          </p>
          ...
          <textarea
            value={form.solution?.[codeLang]||''}
            onChange={e=>setSolutionLang(codeLang,e.target.value)}
            rows={16}
            placeholder={codeLang==='cpp'
              ? '#include <bits/stdc++.h>\nusing namespace std;\n\nint main() {\n    // your solution\n    return 0;\n}'
              : codeLang==='python'
              ? 'import sys, json\n\ndef solve(input):\n    pass\n\nraw = sys.stdin.read().strip()\nprint(json.dumps(solve(json.loads(raw))))'
              : 'import java.util.*;\n\npublic class Main {\n    public static void main(String[] args) {\n        // your solution\n    }\n}'}
            className="w-full px-3 py-2 text-sm font-mono bg-game-surface border border-game-border rounded-xl
              text-slate-300 outline-none resize-none focus:border-purple-500/60 transition-colors"
            style={{ fontFamily: 'monospace' }}
          />
          <p className="text-xs text-slate-700">
            💡 Solutions are stored in Firestore directly. You can also push them to your GitHub CodeZ repo for version control.
          </p>
        </div>
      )}
      ...
      {activeTab==='comments' && (
        <div className="space-y-4">
          <Field label="Intuition (the key insight, 2-3 sentences)">
            <textarea value={form.comment?.intuition||''} onChange={e=>setComment('intuition',e.target.value)}
              rows={3} placeholder="What is the core insight that makes this problem tractable?"
              className={textareaCls}/>
          </Field>
          <Field label="Approach (step by step)">
            <textarea value={form.comment?.approach||''} onChange={e=>setComment('approach',e.target.value)}
              rows={4} placeholder="Describe the algorithm step by step…"
              className={textareaCls}/>
          </Field>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs text-slate-500">Walkthrough (trace through an example)</label>
              <button onClick={addWalkLine}
            ...
            <div className="space-y-1.5">
              {(form.comment?.walkthrough||[]).map((line,i)=>(
                <div key={i} className="flex items-center gap-2">
                  ...
                  <input value={line} onChange={e=>setWalkLine(i,e.target.value)}
                    placeholder={`e.g. "i=0: complement=7, not in map, store {2:0}"`}
                    className="flex-1 px-2 py-1.5 text-xs font-mono bg-game-surface border border-game-border
                      rounded-lg text-slate-300 outline-none focus:border-purple-500/60"/>
                  <button onClick={()=>removeWalkLine(i)}
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Time Complexity">
              <input value={form.comment?.complexity?.time||''} onChange={e=>setComplexity('time',e.target.value)}
                placeholder="e.g. O(n)" className={inputCls}/>
            </Field>
            <Field label="Space Complexity">
              <input value={form.comment?.complexity?.space||''} onChange={e=>setComplexity('space',e.target.value)}
                placeholder="e.g. O(n)" className={inputCls}/>
            </Field>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs text-slate-500">Common Mistakes</label>
              <button onClick={()=>setComment('commonMistakes',[...(form.comment?.commonMistakes||[]),''])}
                className="text-xs text-purple-400 hover:text-purple-300 flex items-center gap-1">
                <Plus className="w-3 h-3"/>Add
              </button>
            </div>
            <div className="space-y-1.5">
              {(form.comment?.commonMistakes||[]).map((m,i)=>(
                <div key={i} className="flex items-center gap-2">
                  <span className="text-red-500 shrink-0">✗</span>
                  <input value={m}
                    onChange={e=>{
                      const arr=[...(form.comment?.commonMistakes||[])];
                      arr[i]=e.target.value;
                      setComment('commonMistakes',arr);
                    }}
                    placeholder="Describe the common mistake and why it's wrong"
                    className="flex-1 px-2 py-1.5 text-xs bg-game-surface border border-game-border
                      rounded-lg text-slate-300 outline-none focus:border-purple-500/60"/>
                  <button onClick={()=>setComment('commonMistakes',(form.comment?.commonMistakes||[]).filter((_,idx)=>idx!==i))}
                    className="text-slate-700 hover:text-red-400 transition-colors shrink-0">
                    <X className="w-3 h-3"/>
                  </button>
                </div>
        ...
        <div className="space-y-3">
          <div className="p-3 rounded-xl bg-yellow-500/10 border border-yellow-500/20">
            <p className="text-xs text-yellow-400 leading-relaxed">
              Hints are shown one at a time in the Solution tab when AI is off, or supplement the AI hints when on.
              Order them from most subtle to most revealing. Each hint should be a single sentence.
            </p>
          </div>
          ...
          <button onClick={addHint}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs
              bg-game-surface border border-game-border text-slate-500 hover:text-slate-300 transition-colors">
            <Plus className="w-3.5 h-3.5"/>Add Hint
          </button>

          <p className="text-xs text-slate-700">
            Tip: Aim for 3 hints. First = category/data structure. Second = key insight. Third = near-solution nudge.
          </p>
        </div>
      )}
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-game-border">
        ...
        <button onClick={onCancel}
          className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm
            bg-game-surface border border-game-border text-slate-400 hover:text-slate-200 transition-colors">
          <X className="w-3.5 h-3.5"/>Cancel
        </button>
        <span className="text-xs text-slate-700 ml-2">
          Changes reflect on website instantly via Firestore
        </span>
      </div>
    </div>
  );
...
// ── Topic group ────────────────────────────────────────────────
function TopicGroup({ topic, problems, onEdit, onDelete, deletingId }) {
  const [open, setOpen] = useState(true);
  return (
    <div className="mb-3">
      <button onClick={()=>setOpen(o=>!o)}
        className="w-full flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-game-surface transition-colors text-left">
      ...
      {open && (
        <div className="ml-2 space-y-1.5 mt-1">
          {problems.map(p=>(
            <div key={p.id}
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-game-surface border border-game-border">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={()=>onEdit(p)}
                  className="p-1.5 rounded-lg text-slate-600 hover:text-purple-400 hover:bg-purple-500/10 transition-colors">
...
export default function AdminPanel() {
  ...
  async function handleDelete(problem) {
    ...
    try { await deleteProblem(problem.id); showToast('Deleted.'); await loadProblems(); }
    catch(e) { showToast('Delete failed: '+e.message,'error'); }
    finally { setDeletingId(null); }
  }

  async function handleSeed() {
    if (problems.length>0 && !window.confirm(
      `Firestore already has ${problems.length} problems. Seeding will add more. Continue?`
    )) return;
    setSeeding(true);
    try {
  ...
  return (
    <div className="min-h-screen" style={{ background: isMario ? '#5c94fc' : '#0f0e17' }}>
      ...
      <header className="sticky top-0 z-40 px-6 py-3 flex items-center justify-between"
        ...
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-lg bg-purple-600/30 border border-purple-500/40
            flex items-center justify-center">
            <span className="text-sm">⚙</span>
          </div>
          <div>
            <p className="pixel text-xs" style={{color: isMario ? '#fbd000' : '#a78bfa'}}>
              Admin Panel
            </p>
            <p className="text-xs" style={{color: isMario ? '#ffe4b5' : '#64748b'}}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={toggle}
            ...
            style={isMario
              ? {background:'#8b2e00',border:'2px solid #5a1a00',color:'#ffd000'}
              : {background:'var(--game-surface)',border:'1px solid var(--game-border)',color:'#a78bfa'}}>
            {isMario ? '🌙 Dark' : '🌞 Mario'}
          </button>
          <button onClick={logout}
            className="px-3 py-1.5 text-xs bg-game-surface border border-game-border rounded-lg
      ...
      {confirmDelete && (
          <div className="mb-4 flex items-center gap-3 p-3 rounded-xl bg-red-500/10 border border-red-500/30">
            ...
            <button onClick={()=>setConfirmDelete(null)}
              className="px-3 py-1.5 bg-game-surface border border-game-border text-slate-400 text-xs rounded-lg">
              Cancel
            </button>
          </div>
        )}
          ...
          <div className="flex items-center gap-3 mb-5 flex-wrap">
            ...
            {problems.length===0 && (
              <button onClick={handleSeed} disabled={seeding}
                ...
                {seeding?<Loader className="w-4 h-4 animate-spin"/>
                 :seedDone?<Check className="w-4 h-4 text-green-400"/>
                 :<Server className="w-4 h-4"/>}
                Seed Built-in ({BUILT_IN_PROBLEMS.length})
              </button>
            )}
            <button onClick={loadProblems} disabled={loading}
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)}
              ...
              className="flex-1 min-w-36 px-3 py-2 text-sm bg-game-surface border border-game-border
                rounded-xl text-slate-300 placeholder-slate-700 outline-none
        ...
        {(mode==='create'||mode==='edit') && (
          <div>
            <div className="flex items-center gap-2 mb-5">
              <button onClick={()=>{setMode('list');setEditingProblem(null);}}
                className="text-slate-600 hover:text-slate-300 transition-colors text-sm">
                ← Back
              </button>
              <h2 className="text-white font-semibold">
                {mode==='create'?'New Problem':`Edit: ${editingProblem?.title}`}
              </h2>
            </div>
            <ProblemForm
              initial={mode==='edit'?editingProblem:null}
              onSave={handleSave}
          ...
          loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-slate-700 text-sm">
                Click "Seed Built-in Problems" or "New Problem" to get started.
              </p>
            </div>
          ) : (
            <>

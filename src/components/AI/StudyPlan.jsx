...
export default function StudyPlan() {
  ...
  async function fetchPlan() {
    ...
    try {
      // uid removed — backend gets it from the Authorization token
      var data = await authFetch('/ai/studyplan', {
        method: 'POST',
        body: JSON.stringify({
  ...
  if (error) {
    return (
      <div className="game-card p-4 flex items-center justify-between">
        ...
        <button onClick={fetchPlan} className="text-xs text-slate-500 hover:text-slate-300 transition-colors">
          Retry
        </button>
      </div>
    );
    ...
    <AIGate>
      <div className="game-card overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          {plan.greeting && <p className="text-sm text-slate-300 leading-relaxed">{plan.greeting}</p>}
          {plan.focus && (
          ...
          {plan.plan?.length > 0 && (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                <Calendar className="w-3.5 h-3.5"/> 3-Day Plan
              </p>
              {plan.plan.map(function(day, i) {
                return (
                  <div key={i} className={`rounded-xl border p-3.5 ${DAY_COLORS[i % DAY_COLORS.length]}`}>
                    ...
                    {day.why && <p className="text-xs text-slate-600 ml-[4.5rem] mt-0.5 italic">{day.why}</p>}
                  </div>
                );
              })}
            </div>
